...
- `AI_SUMMARY_MODEL`: Model name for narrative summaries. Default: `Qwen/Qwen3-Coder-Next-FP8`.
...
- `narrative_top_k`: System-wide top-K message retrieval for narrative updates.
- `dashboard_show_ids`: Show message/narrative IDs in the dashboard when true.
...
`BAAI/bge-base-en-v1.5` outputs **768-dimensional** vectors. If you previously ran FeedEater with a 4096-dim embedding model (e.g. `llama3.1:8b`), the `bus_narratives.embedding` and `mod_slack.slack_message_embeddings.embedding` vector columns must be recreated at the new dimension:

```sql
ALTER TABLE bus_narratives DROP COLUMN IF EXISTS embedding;
ALTER TABLE mod_slack.slack_message_embeddings DROP COLUMN IF EXISTS embedding;
```

The worker's `ensureNarrativeStorage` function will recreate `bus_narratives.embedding` automatically on next start. The `slack_message_embeddings` column is recreated by the Slack module migrations. Existing stored embeddings will be regenerated naturally on the next `collect` and `updateNarratives` cron runs.

### One-time rename from “context” → “narrative”

If upgrading an existing database, run the SQL in [`packages/db/migrations/manual/001_context_to_narrative.sql`](../packages/db/migrations/manual/001_context_to_narrative.sql) **before** starting services that expect `bus_narratives` and the new setting keys.
